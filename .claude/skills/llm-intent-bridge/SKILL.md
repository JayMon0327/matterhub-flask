...
3. **switch 개수가 3을 초과하면 `detect_idx()`의 정규식 `r'([1-3])\s*번'`과 한국어 단어 매핑(`첫/하나/일번/둘째/이번/셋째/삼번`)도 함께 확장**. 안 늘리면 "4번 켜줘" 같은 발화가 idx=None으로 떨어져 전체 켜기로 잘못 동작하거나 그대로 LLM 통과로 흘러가는 silent 누락이 생긴다.
4. (재시작 불필요 — 인터랙티브 종료 후 다시 실행하면 반영)
